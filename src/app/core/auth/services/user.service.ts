...
export class UserService {
  ...
  getUsers(config: UserListConfig): Observable<{ users: User[]; usersCount: number }> {
    ...
    return this.http.get<{ users: User[]; usersCount: number }>(
      "/user/users", { params });
  }
  ...
  delete(email: string): Observable<void> {
    return this.http.delete<void>(`/user/${email}`);
  }
